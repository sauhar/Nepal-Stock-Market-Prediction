# -*- coding: utf-8 -*-
"""
Created on Thu Jun 25 09:47:13 2019
...
def adbl(request):
        ...
        def getfromdatabase():
            ...
            cur = conn.cursor()
            cur.execute("""select openprice,maxprice,minprice,closingprice,date from stockdatacopy where  symbol = 'PLIC' order by date;""")
            row=cur.fetchall()
            conn.commit()
            cur.close()
            #print(row)
