...
class prediction:
    ...
    def execute(company):
        ...
        data = fromdatabase.getfromdatabase(company)
       ...
       # plt.show()
        ...
        tstep = 30
        # since we are looking 30 timesteps back, we can start looping over only after 30th record in our training set
        data = []
        ...
        ax.set_title('Stock Price Prediction of ' + company)
        ax.set_xlabel('Date')
        ...
        fileName = company + '_' + str(prediction().getCurrentTime()) +'.jpeg';
        
        plt.savefig('D:/sauhar/major project/project/nsap/home/static/home/img' + fileName, format='jpeg')
        
        # confusion matrix


        print("confusionMatrix : ", confusion_matrix(y_test[:, 0].astype(int), y_pred[:, 0].astype(int)))
        #print("Accuracy Score : ", accuracy_score(y_test[:, 0].astype(int), y_pred[:, 0].astype(int)))
        print("confusionMatrix : ",classification_report(y_test[:, 0].astype(int), y_pred[:, 0].astype(int)))
        #plt.show()
        ...
        #print(x_train)
        prediction.insertResult(company,fileName, normalized_rmse, predictedtomorrow)
  
    def getCurrentTime(self):
        return int(time.time()) 

    def insertResult(company, fileName, nrms, pv):
        date = time.strftime('%Y-%m-%d')
        conn = connection.connectDatabase()
        cur = connection.getCur(conn)
        print('writing to the database')
        print('Value of nrms :', float(nrms))
        cur.execute("insert into result(company,filename,nrmse,predicted_value,date) values (%s,%s,%s,%s,%s)",(company, fileName,float(nrms), float(pv), date))
        conn.commit()
        cur.close()
